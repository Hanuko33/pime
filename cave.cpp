...
void generate_cave(chunk* chunk, int entrance_x, int entrance_y)
{
	...
	for (int i=0; i<CHUNK_SIZE; i++)
	{
		...
		{
			chunk->table[2][i][j].tile=TILE_CAVE_WALL;
		}
	}
	for (int i=0; i<(CHUNK_SIZE*(CHUNK_SIZE*3)); i++)
    {
        ...
        if (!y_x_axis)
		{
			...
			chunk->table[2][gen_point_y][gen_point_x].tile = TILE_CAVE_FLOOR;
            if (!plus_minus)
			{
    ...
    chunk->table[2][entrance_y][entrance_x].tile = TILE_CAVE_DOOR;
}
